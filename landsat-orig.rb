...
class Scene
  ...
  def unzip
    `cd #{@outdir} && gtar --transform 's/^.*_/#{id}_/g' -xzvf #{id}.tar.bz`
  end
  ...
  def warp(polygon_shp)
    ...
    if !processed_files_exist?
      ...
      puts "gdal_merge.py -seperate #{@outdir}/#{band_file_pattern}_{#{satellite.vegetation.join(",")}}-projected.tif -o #{@outdir}/#{band_file_pattern}_RGB-projected.tif"
      `gdal_merge.py -seperate #{@outdir}/#{band_file_pattern}_{#{satellite.vegetation.join(",")}}-projected.tif -o #{@outdir}/#{band_file_pattern}_RGB-projected.tif` #&& \
...
class Processor
  ...
  def get_scenes
    ...
    puts "trying #{url}"
    scenes = Nokogiri::XML(Net::HTTP.get(URI.parse(url))).css('metaData').select {|n| n.children.css('cloudCoverFull').text.to_i < 2 }.map {|n| n.children.css('sceneID').text }
...
if __FILE__ == $0
  ...
  if ARGV.length < 3 || ARGV.length > 3
    puts <<-EOD
      ...
      usage: ruby median-blend.rb start_year end_year outdir

      example: ruby median-blend.rb 1972 2014 /Volumes/Gilese581c/vegas/
    ...
    threads = (1..8).map do
      Thread.new do
        loop do
          ...
          Thread.exit if exit
          scene.download
          scene.warp("#{File.expand_path(File.dirname(__dir__), "..")}/data/POLYGON.shp")
        end
      end
    end
